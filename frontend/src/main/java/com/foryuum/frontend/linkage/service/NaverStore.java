...
public class NaverStore {
	...
	public void getItemInfo (String authToken, Map<String, Object> requestData, Map<String, Object> returnData) {
		try {
	        ...
	        if (response.getStatusCode() == HttpStatus.OK) {
	        	...
	        	convertMap.put("RECEIVER_ADDRES", shippingInfo.getBaseAddress() + " " + shippingInfo.getDetailedAddress());
	...
	public boolean setTrackingNumber (String authToken, Map<String, String> requestData) {
		...
		try {
			...
			jsonObject.put("productOrderId", requestData.get("P_PRODUCT_ORDER_ID"));
			jsonObject.put("deliveryMethod", "DELIVERY");
			jsonObject.put("deliveryCompanyCode", "HANJIN");
			jsonObject.put("trackingNumber", requestData.get("TRACKING_NUMBER"));
			jsonObject.put("dispatchDate", formattedDate);

			JSONObject requestJson = new JSONObject();
			List<JSONObject> productOrderIds = Arrays.asList(jsonObject);
			requestJson.put("dispatchProductOrders", productOrderIds.toString());
